...
public class SquigglySpan extends ReplacementSpan {
    ...
    SquigglySpan(Resources resources) {
        ...
        squigglyPaint.setColor(resources.getColor(R.color.monitor_leak));
        float strokeWidth =
                resources.getDimensionPixelSize(R.dimen.monitor_squiggly_span_stroke_width);
        squigglyPaint.setStrokeWidth(strokeWidth);

        halfStrokeWidth = strokeWidth / 2;
        amplitude = resources.getDimensionPixelSize(R.dimen.monitor_squiggly_span_amplitude);
        periodDegrees =
                resources.getDimensionPixelSize(R.dimen.monitor_squiggly_span_period_degrees);
        path = new Path();
        float waveHeight = 2 * amplitude + strokeWidth;
        halfWaveHeight = waveHeight / 2;
        referenceColor = resources.getColor(R.color.monitor_reference);
    }
